...
ADMIN_PAGES = {
    "Dashboard":    {"key": "dashboard",    "icon": "ti-layout-dashboard", "group": "main"},
    "Chatbot":      {"key": "chat",         "icon": "ti-message-circle",   "group": "main"},
    "Analytics":    {"key": "analytics",    "icon": "ti-chart-bar",        "group": "main"},
    "Signalements": {"key": "signalements", "icon": "ti-alert-triangle",   "group": "manage"},
    "Utilisateurs": {"key": "users",        "icon": "ti-users",            "group": "manage"},
    "Paramètres":   {"key": "settings",     "icon": "ti-settings",         "group": "manage"},
}
...
.sb-divider { height: 0.5px; background: rgba(0,0,0,.07); margin: 6px 2px; }
...
.sb-item.active { background: #FEF3E2; }
.sb-item.active .sb-label { color: #633806; font-weight: 600; }
.sb-item.active i { color: #854F0B; }
.sb-dot {
    margin-left: auto; width: 6px; height: 6px;
    border-radius: 50%; background: #E07B2A; flex-shrink: 0;
}
...
.sb-badge {
    ...
    background: #FAEEDA; color: #633806; margin-top: 3px;
}
.sb-logout {
...
def render_admin_sidebar() -> str:
    with st.sidebar:
        ...
        if "nav" in params:
            nav_val    = params["nav"]
            valid_keys = [v["key"] for v in ADMIN_PAGES.values()]
            if nav_val in valid_keys and nav_val != st.session_state.admin_page:
                st.session_state.admin_page = nav_val
                st.query_params.clear()
                st.rerun()
            else:
                st.query_params.clear()
        ...
        nav_items  = ""
        last_group = None
        for label, cfg in ADMIN_PAGES.items():
            key    = cfg["key"]
            icon   = cfg["icon"]
            group  = cfg["group"]
            ...
            if last_group and group != last_group:
                nav_items += '<div class="sb-divider"></div>'
        ...
        nom       = st.session_state.get("nom", "Administrateur")
        email     = st.session_state.get("email", "")
        initiales = "".join(w[0].upper() for w in nom.split()[:2]) if nom else "A"
        ...
        <div class="sb-wrap">
            <div class="sb-head">
                ...
                <div>
                    <div class="sb-title">TDE Admin</div>
